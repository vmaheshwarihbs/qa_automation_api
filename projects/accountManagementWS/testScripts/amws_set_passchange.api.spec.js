...
describe('AMWS- Set Password Change', () => {
    ...
    it('TC-001 - setpasswordchange,it should change the password status of person account with 200 status code', (done) => {
        request.post(data.TC001.endpoint)
            ...
            .end((err, res) => {
                ...
                logger.info("TC-001 -setpasswordchange - Request: ", res.request); //Logging request
                logger.info("TC-001 -setpasswordchange - Response: ", res.text); // Logging response
                done();
            });
    });
    ...
    it('TC-003 - setpasswordchange,it should change the password status of person account with 200 status code', (done) => {
        request.post(data.TC003.endpoint)
            .send({ "username" : "vmaheshwari@hbsstg.org","requirePasswordChangeFlag" : "true"  })
            .set("Content-Type", "application/json")
            .set("accept", "application/json")
            .end((err, res) => {
                if (err) done.fail(err);
