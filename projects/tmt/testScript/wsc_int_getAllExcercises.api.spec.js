const superTest = require('supertest'),
    ...
    data = require('../testData/wsc_int_getAllExercises'),
    Joi = require('joi');
...
describe('GET_AllExercises', () => {
    ...
    it('TC-006 - All Exercises - validating data with database', (done) => {
        request.post(data.TC006.endpoint)
            ...
            .end(async(err, res) => {
                ...
                try{
                   ...
                   let result =  await connection.execute('select * from exercise');
                   // console.log(result)
                   let present = []
                   let not_present = []
                   ...
                   fs.writeFileSync('./database_response/int_get_present.json', p_data);
                   let np_data = JSON.stringify(not_present);
                   fs.writeFileSync('./database_response/int_get_not_present.json', np_data);
                 }
                 catch (err) {
